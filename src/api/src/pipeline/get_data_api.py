...
class GetDataApi:
    ...
    def get_data_api_thunders(self, thundres, p_urls):
        conn = ConsultaThundersApi()               
        try:
            #print(f'Buscando Informações')
            token = conn.get_token(thundres)
    ...
    #     path = f'src/sql/script_get_data_MeasuringPoint.sql'
    #     sql_alert = util.read_file_sql(path)


    #     # TRATAR O DATAFRAME RETORNO DA QUERY DO ALERTA
    #     df = pd.DataFrame()
    #     df = pd.read_sql_query(sql_alert, engine)
    #     #df = df.loc[df['Thunders']==Thunders]
