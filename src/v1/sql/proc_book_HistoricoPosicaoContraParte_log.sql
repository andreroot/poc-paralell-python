...
DECLARE @it AS INT = 0;
	...
	WHILE @it < @n_datas
        ...
        IF OBJECT_ID('tempdb..#TempPosContParTable2') IS NOT NULL
        BEGIN
            DROP TABLE #TempPosContParTable2
        END
        -- Tabela com os preços Médios e com as separações de Fixo/Variavel/Collar/Opção
        SELECT 
            @DataHistorico AS DataHistorico
            ,* INTO #TempPosContParTable2
        FROM (
            ...
            ) TempPosContParTable2
        --SELECT * FROM #TempTable2

            SET @it = @it + 1;

            DELETE FROM book.book.HistoricoPosicaoContraparte_log where DataHistorico = @dataHistorico;

            INSERT INTO book.book.HistoricoPosicaoContraparte_log
                SELECT * FROM #TempPosContParTable2;
