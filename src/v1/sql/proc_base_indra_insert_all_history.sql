--> alteração no script para incluir novas fontes de energia: 08/03
...
--> executar retroativo
--SET @DataHistorico = (select CONVERT(DATE, DATEADD(DAY, -8, max(data)), 103) start_date from book.curva.Curva_Fwd where curva = 'Oficial');
		...
		END
			...
			SELECT 
								...
								,CASE WHEN A.[energySourceDescription] = 'Convencional' THEN 'Convencional'
									...
									WHEN A.[energySourceDescription] = 'Incentivada Não Especial 50%' Then 'INE5'
									WHEN A.[energySourceDescription] = 'Incentivada 80%' Then '80% Incent.'
									WHEN A.[energySourceDescription] = 'Incentivada Não Especial 100%' Then 'INE1'							
									ELSE 'Não Previsto'
								END																																																									AS FonteEnergia	
								...
								,CASE
										WHEN A.[classifications] = 'Curto Prazo'
												OR  A.[classifications] = 'Longo Prazo'  THEN 'Trading'
										...
										WHEN A.[classifications] = 'op PF'  THEN 'PF_Marcio'
										Else 'Trading'
								END																																																									AS Portfolio	
							...
							FROM BookIndra.dbo.s_all_operations(@DataHistorico) AS A
							) TempTableAllHistory;
